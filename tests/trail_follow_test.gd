...
func _ready() -> void:
	...
	# 일직선 이동: 동료는 trail_spacing_frames 만큼 뒤를 따른다
	var step := Vector2(2, 0)
	for k in 60:
		party.global_position += step
		party._update_trail()
	var spacing: int = party.trail_spacing_frames
	var expected_lag := step.length() * spacing
	...
	_check(abs(actual_lag - expected_lag) < 0.5, "지연 거리 = %d프레임×%.0f = %.0f (실제 %.0f)" % [spacing, step.length(), expected_lag, actual_lag])
