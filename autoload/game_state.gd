...
const FORMATION_DIR := "res://data/formations"
...
var unlocked_regions: Array[StringName] = [&"stage_meadow"] # 이동 가능한 지역 (core 지역 노드로 해금, recalc가 재구성)
var party_in_town: bool = false        # 파티가 마을 안에 있는가 (지역 이동 게이트 — 필드가 갱신)
var pending_travel: StringName = &""   # 예약 이동 목적지 (자동이동으로 마을 도착 시 실행)
...
const _STAT_ADD := {
	...
	"hero_max_hp": 0, "max_enemies_bonus": 0,
}
const _STAT_BOOL := {
	...
	# ─ 파티 (R2) ─
	"mage_aoe_enabled": false, "status_care_enabled": false, "full_party_unlocked": false,
	# ─ 트링켓 시스템 해금 플래그 ─
...
var equipped_trinkets: Array[StringName] = [] # 장착 중 (member_trinkets에서 파생 — 세트/수집/저장 호환용)
var member_trinkets: Dictionary = {}          # 파티원 슬롯 (R4): role(StringName) -> Array[StringName] (파생)
var manual_trinkets: Dictionary = {}          # 수동 장착 고정 (R5 카탈로그): role -> Array[StringName]
var monster_defs: Dictionary = {}       # id(StringName) -> MonsterData (전체 도감 — 포메이션 조회용)
var formation_catalog: Array[EncounterFormationDef] = [] # 인카운터 포메이션 (무리 전투)
var region_kills: Dictionary = {}       # region_id -> 그 지역 누적 처치 수 (적 다양화 해금)
var survey: Dictionary = {}             # region_id -> 조사도 0~1 (포메이션 클리어로 증가)
## 지역 번호 → 인카운터당 최대 적 수 바닥값 (마법사/킬로 더 오를 수 있음). [meadow,forest,cave,castle,final]
const _REGION_MAX_ENEMIES := {1: 1, 2: 3, 3: 4, 4: 5, 5: 6}
const _GROUP_KILLS_FOR_2 := 8           # 초원 적 N마리 처치 시 최대 2마리 인카운터 해금
...
## 어느 단계로든 실제 전환(역행 포함, 게이트 없음 — 내부 공용). 마을 밖 필드만 바뀐다.
## 첫 도달이면 동료 합류 + full_heal. region_changed 발신.
func _set_stage(stage_id: StringName) -> void:
	var target := _stage_by_id(stage_id)
	if target == null or stage_id == current_region:
		return
...
## 특정 단계로 전진(역행 불가) — 테스트/디버그용 저수준 API.
...
func region_unlocked(stage_id: StringName) -> bool:
	return stage_id in unlocked_regions


func stage_display_name(stage_id: StringName) -> String:
	var s := _stage_by_id(stage_id)
	return s.display_name if s != null else String(stage_id)


## 지역 이동 시도. 마을 안 + 해금된 지역만. 반환 {ok, msg}.
func travel_to_region(stage_id: StringName) -> Dictionary:
	if not region_unlocked(stage_id):
		return {"ok": false, "msg": Locale.t("아직 갈 수 없는 지역입니다.")}
	if stage_id == current_region:
		return {"ok": false, "msg": Locale.t("이미 그 지역에 있습니다.")}
	if not party_in_town:
		return {"ok": false, "msg": Locale.t("지역 이동은 마을 안에서만 가능합니다.")}
	_set_stage(stage_id)
	pending_travel = &""
	return {"ok": true, "msg": Locale.t("%s로 이동했다.") % stage_display_name(stage_id)}


## 예약 이동 — 마을 밖에서 지역을 고르면 자동이동으로 마을에 돌아간 뒤 이동한다.
## 자동이동 해금 전이면 실패. party.gd가 마을 도착 시 _try_pending_travel을 호출한다.
func reserve_travel(stage_id: StringName) -> Dictionary:
	if not region_unlocked(stage_id):
		return {"ok": false, "msg": Locale.t("아직 갈 수 없는 지역입니다.")}
	if not auto_hunt_unlocked: # 자동 이동(전투 대기열)이 있어야 예약 가능
		return {"ok": false, "msg": Locale.t("지역 이동은 마을 안에서만 가능합니다.")}
	pending_travel = stage_id
	return {"ok": true, "msg": Locale.t("마을로 돌아가 %s로 이동합니다.") % stage_display_name(stage_id)}


## 마을 도착 시(party.gd) 호출 — 예약된 이동이 있으면 실행.
func try_pending_travel() -> void:
	if pending_travel != &"" and party_in_town:
		travel_to_region(pending_travel)


## 이동 가능한 다음 해금 지역 (표지판 순환용). 현재 지역 다음, 없으면 첫 해금 지역.
func next_unlocked_region() -> StringName:
	var ordered := unlocked_regions.duplicate()
	ordered.sort_custom(func(a: StringName, b: StringName) -> bool:
		return _stage_index_of(a) < _stage_index_of(b))
	var idx := ordered.find(current_region)
	if idx >= 0 and ordered.size() > 1:
		return ordered[(idx + 1) % ordered.size()]
	return ordered[0] if not ordered.is_empty() else &"stage_meadow"


func _stage_index_of(stage_id: StringName) -> int:
	var s := _stage_by_id(stage_id)
	return s.index if s != null else 0
...
## 현재 파티원 역할 목록 (트링켓 슬롯 키 — hero + 동료 role). 풀파티면 [hero,warrior,mage,priest].
func member_roles() -> Array[StringName]:
	var roles: Array[StringName] = [&"hero"]
	for c in companions:
		roles.append(c.role)
	return roles


## 파티원 슬롯에 owned 트링켓을 배치 (R4/R5). 멤버당 trinket_slots()개.
## 순서: ① 수동 고정(manual_trinkets) ② 친화 일치 자동 ③ 남은 것으로 빈 슬롯.
## equipped_trinkets는 여기서 파생된다(세트/수집/저장 호환).
func _autoequip_member_trinkets() -> void:
	var per_member := trinket_slots() # 멤버당 슬롯(잠금 시 0)
	var roles := member_roles()
	member_trinkets.clear()
	for r: StringName in roles:
		member_trinkets[r] = [] as Array[StringName]
	equipped_trinkets = []
	if per_member <= 0:
		return
	var taken := {} # tid -> true (이미 배치된 트링켓)
	# ① 수동 고정 (owned + 현재 멤버 + 슬롯 한도)
	for r: StringName in roles:
		for tid: StringName in manual_trinkets.get(r, []):
			if member_trinkets[r].size() >= per_member:
				break
			if tid in owned_trinkets and not taken.has(tid):
				member_trinkets[r].append(tid)
				taken[tid] = true
	# ② 친화 일치 자동
	for r: StringName in roles:
		for tid: StringName in owned_trinkets:
			if member_trinkets[r].size() >= per_member:
				break
			if taken.has(tid):
				continue
			var trk: TrinketData = trinket_catalog.get(tid)
			if trk != null and trk.affinity_role == r:
				member_trinkets[r].append(tid)
				taken[tid] = true
	# ③ 남은 트링켓으로 빈 슬롯 채움
	for r: StringName in roles:
		for tid: StringName in owned_trinkets:
			...
			taken[tid] = true
	for r: StringName in roles:
		equipped_trinkets.append_array(member_trinkets[r])


## 수동 장착 (카탈로그 UI): tid를 role 멤버에게 고정. 다른 멤버에서 빼고, 슬롯 초과면 가장 오래된 것 밀어냄.
func equip_trinket_on(role: StringName, tid: StringName) -> bool:
	if tid not in owned_trinkets or role not in member_roles():
		return false
	for r: StringName in manual_trinkets.keys(): # 다른 멤버 고정에서 제거
		manual_trinkets[r] = (manual_trinkets[r] as Array).filter(func(t: StringName) -> bool: return t != tid)
	var arr: Array = manual_trinkets.get(role, [])
	arr = arr.duplicate()
	arr.append(tid)
	var per := trinket_slots()
	if per > 0 and arr.size() > per:
		arr = arr.slice(arr.size() - per) # 최신 per개만 유지
	manual_trinkets[role] = arr
	recalculate_stats()
	return true


## 수동 고정 해제 (해당 트링켓을 다시 자동 배치 대상으로).
func unequip_trinket(tid: StringName) -> void:
	for r: StringName in manual_trinkets.keys():
		manual_trinkets[r] = (manual_trinkets[r] as Array).filter(func(t: StringName) -> bool: return t != tid)
	recalculate_stats()


## 모든 수동 고정 해제 → 전부 자동 배치로.
func clear_manual_trinkets() -> void:
	manual_trinkets.clear()
	recalculate_stats()
...
func _load_monster_defs() -> void:
	...
	for file in dir.get_files():
		...
		if not fname.ends_with(".tres"):
			continue
		var m: MonsterData = load(MONSTER_DIR + "/" + fname)
		if m:
			monster_defs[m.id] = m


func _load_formation_catalog() -> void:
	formation_catalog.clear()
	var dir := DirAccess.open(FORMATION_DIR)
	if dir == null:
		return
	for file in dir.get_files():
		var fname := file.trim_suffix(".remap")
		if not fname.ends_with(".tres"):
			continue
		var f: EncounterFormationDef = load(FORMATION_DIR + "/" + fname)
		if f:
			formation_catalog.append(f)


func monster_by_id(id: StringName) -> MonsterData:
	return monster_defs.get(id)


func formation_def(id: StringName) -> EncounterFormationDef:
	for f: EncounterFormationDef in formation_catalog:
		if f.id == id:
			return f
	return null


## 도감에 등록된(한 번이라도 마주친) 적인가. ensure_hunt_entry가 monster_catalog를 채운다.
func is_monster_discovered(id: StringName) -> bool:
	return monster_catalog.has(id)


func region_kill_count(region: StringName) -> int:
	return int(region_kills.get(region, 0))


func survey_of(region: StringName) -> float:
	return float(survey.get(region, 0.0))


func add_region_kill(region: StringName) -> void:
	region_kills[region] = region_kill_count(region) + 1


## 조사도 증가 (포메이션 전멸 보상). 1.0에서 멈춘다.
func add_survey(region: StringName, amount: float) -> void:
	if amount <= 0.0:
		return
	var v := minf(1.0, survey_of(region) + amount)
	if v != survey_of(region):
		survey[region] = v
		EventBus.stats_changed.emit() # 조사도 변화 → 적/포메이션 해금 갱신


## 해금 조건 사전을 현재 진행도와 비교 (전부 충족 = AND). 빈 사전이면 항상 true.
## 지원 키: region_kills(int) · survey(0~1) · discovered(적 id) · monster_kills({id:n}) · requires_flag(스탯 불 키)
func _condition_met(cond: Dictionary) -> bool:
	if cond.is_empty():
		return true
	if cond.has("region_kills") and region_kill_count(current_region) < int(cond["region_kills"]):
		return false
	if cond.has("survey") and survey_of(current_region) < float(cond["survey"]):
		return false
	if cond.has("discovered") and not is_monster_discovered(StringName(cond["discovered"])):
		return false
	if cond.has("monster_kills"):
		for mid: Variant in cond["monster_kills"]:
			if kills(StringName(mid)) < int(cond["monster_kills"][mid]):
				return false
	if cond.has("requires_flag") and not bool(stat(String(cond["requires_flag"]))):
		return false
	return true


func formation_unlocked(def: EncounterFormationDef) -> bool:
	return _condition_met(def.unlock)


func enemy_unlocked(data: MonsterData) -> bool:
	return data != null and _condition_met(data.unlock)


## 인카운터당 최대 적 수 — 지역 바닥값 + 초원 킬 해금 + 마법사 + 트링켓 보정, [1,6].
func max_enemies_per_encounter() -> int:
	var me := 1
	if region_kill_count(current_region) >= _GROUP_KILLS_FOR_2:
		me = 2
	if has_companion(&"mage"): # 마법사 합류 시 최소 3 (R2)
		me = maxi(me, 3)
	me = maxi(me, int(_REGION_MAX_ENEMIES.get(region_number(), 1)))
	me += int(stat("max_enemies_bonus")) # 마물 피리(+1)·겁쟁이 향(-1) 등 (R4)
	return clampi(me, 1, 6)
...
func roll_encounter_formation(primary_data: MonsterData) -> Dictionary:
	...
	var total := 0.0
	for f: EncounterFormationDef in formation_catalog:
		if f.region_id == current_region and f.spawn_weight > 0.0 and formation_unlocked(f):
			pool.append(f)
			total += f.spawn_weight
	if pool.is_empty() or total <= 0.0:
		return {"datas": ([primary_data] if primary_data else []), "formation_id": &""}
	var r := randf() * total
	var chosen: EncounterFormationDef = pool[pool.size() - 1]
	for f: EncounterFormationDef in pool:
		r -= f.spawn_weight
		if r <= 0.0:
			chosen = f
			break
	return {"datas": _build_formation_enemies(chosen), "formation_id": chosen.id}


## 포메이션 정의 → 실제 적 MonsterData 배열 (count 범위 추첨, max_enemies로 클램프).
func _build_formation_enemies(def: EncounterFormationDef) -> Array:
	var maxn := max_enemies_per_encounter()
	var datas: Array = []
	for slot: Variant in def.enemy_slots:
		var md: MonsterData = monster_by_id(StringName(slot["enemy_id"]))
		if md == null:
			continue
		var n := randi_range(int(slot.get("count_min", 1)), int(slot.get("count_max", 1)))
		for i in n:
			if datas.size() >= maxn:
				break
			datas.append(md)
...
func upgrades_for_axis(axis: String) -> Array[UpgradeData]:
	...
	for upgrade: UpgradeData in catalog.values():
		...
		if upgrade.branch in ["core", "village", "command", "trinket", "bridge", "infinite", "party"]:
			continue
...
## + 노드별 진행도 해금 조건(unlock: 영입 조건 등)도 충족해야 한다.
func node_unlocked(upgrade: UpgradeData) -> bool:
	if not _condition_met(upgrade.unlock):
		return false # 영입 조건 등 미충족 (예: 초원 8킬 전 전사 영입 잠김)
...
func purchase(upgrade: UpgradeData) -> bool:
	...
	if upgrade.effects.has("unlock_region"):
		EventBus.show_toast.emit(Locale.t("새 길이 열렸다 — 마을 표지판에서 그 지역으로 이동할 수 있다."))
...
func register_kill(data: MonsterData) -> void:
	...
	add_region_kill(current_region) # 지역별 누적 처치 (적 다양화 해금)
...
const MAX_PARTY_SIZE := 4 # 용사 + 동료 3명 (전사/마법사/힐러)

func add_companion(comp: CompanionData) -> void:
	if comp == null or _has_companion(comp.id):
		return
	if member_count() >= MAX_PARTY_SIZE: # 파티 4명 상한
		return
...
func member_max_hp_for(index: int) -> int:
	var base := 0
	if index == 0:
		base = config.hero_max_hp + int(stat("hero_max_hp")) # cmb_hp_* 보너스
	else:
		var ci := index - 1
		base = companions[ci].max_hp if ci < companions.size() else 0
	return int(round(base * float(stat("party_hp_mult")))) # 파티 HP 배율(party_formation_2 등)


## index번째 멤버의 역할 (0=hero, 1+=동료 role). 마법사 광역 판정 등에 쓴다.
func member_role(index: int) -> StringName:
	if index == 0:
		return &"hero"
	var ci := index - 1
	return companions[ci].role if ci < companions.size() else &""
...
func recalculate_stats() -> void:
	...
	group_table = [1.0]
	unlocked_regions = [&"stage_meadow"] # 시작 지역은 항상 이동 가능. core 지역 노드가 추가.
	_init_generic_stats()
	for id: StringName in purchases:
		...
		for key: String in upgrade.effects:
			...
			match key:
				...
				"unlock_region":
					var rid := StringName(value)                    # core 지역 노드 → 이동 가능 지역에 추가
					if rid != &"" and rid not in unlocked_regions:
						unlocked_regions.append(rid)
				_:
					if not _apply_generic_stat(key, value, count):
						push_warning("알 수 없는 효과 키: %s (%s)" % [key, upgrade.id])
	# ─── 장착 트링켓 효과 (업그레이드와 같은 제너릭 스탯 키로 가산) ───
	_autoequip_member_trinkets()
	for role: StringName in member_trinkets:
		for tid: StringName in member_trinkets[role]:
			var trk: TrinketData = trinket_catalog.get(tid)
			if trk == null:
				continue
			for tkey: String in trk.effects:
				_apply_generic_stat(tkey, trk.effects[tkey], 1)
			if trk.affinity_role == role: # 친화 멤버에게 장착 → 추가 보너스
				for bkey: String in trk.affinity_bonus:
					_apply_generic_stat(bkey, trk.affinity_bonus[bkey], 1)
...
## StringName 키 사전 → String 키 사전 (JSON 저장용).
func _sn_dict_out(d: Dictionary) -> Dictionary:
	var out := {}
	for id: Variant in d:
		out[String(id)] = d[id]
	return out
...
## manual_trinkets(role -> Array[StringName])를 저장용 문자열 사전으로.
func _manual_trinkets_out() -> Dictionary:
	var out := {}
	for role: Variant in manual_trinkets:
		out[String(role)] = (manual_trinkets[role] as Array).map(func(s: StringName) -> String: return String(s))
	return out
...
func save_game() -> void:
	...
	var data := {
		...
		"manual_trinkets": _manual_trinkets_out(),
		"region_kills": _sn_dict_out(region_kills),
		"survey": _sn_dict_out(survey),
	}
...
func load_game() -> void:
	...
	manual_trinkets.clear()
	for key: String in data.get("manual_trinkets", {}):
		var arr: Array[StringName] = []
		for t: Variant in data["manual_trinkets"][key]:
			arr.append(StringName(t))
		manual_trinkets[StringName(key)] = arr
	region_kills.clear()
	for key: String in data.get("region_kills", {}):
		region_kills[StringName(key)] = int(data["region_kills"][key])
	survey.clear()
	for key: String in data.get("survey", {}):
		survey[StringName(key)] = float(data["survey"][key])
	materials.clear()
