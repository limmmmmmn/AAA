...
func _ready() -> void:
	...
	# ── 2. 허브 연결 + 비용 ──
	_check(_u(&"core_start").tree_links == [GameState.TREE_CORE], "core_start → 허브")
	...
	_check(GameState.node_unlocked(_u(&"core_start")), "core_start 해금됨(허브 자식)")
	...
	_check(GameState.region_number() == 3, "cave_map 구매 → 동굴(3)")
	GameState.purchase(_u(&"core_castle_shadow"))
	_check(GameState.region_number() == 4, "castle_shadow 구매 → 마왕성 외곽(4)")
	...
	_check(GameState.region_number() == 5, "final_door 구매 → 마왕성 정문(5)")
	GameState.purchase(_u(&"core_ending_machine"))
	_check(GameState.stat("ending_reached") == true, "ending_machine → 엔딩 플래그")

	# ── 6. set_stage는 스탯 경고를 내지 않는다(행동 키) ──
	_check(not GameState.stats.has("set_stage"), "set_stage는 스탯 사전에 안 들어감")
